...
import { createContract } from './../ethereum/crowfundingContract';
...
export class Campaign extends Component {
    ...
    async getCampaign(address) {
        ...
        var isBeneficiaryUser = false;
        if(account != null && beneficiary.toLowerCase() === account.toLowerCase()) { isBeneficiaryUser = true }
            ...
            <Table celled padded color="teal" striped>
                ...
                <Table.Body>
                    <Table.Row>
                        ...
                        <Table.Cell singleLine>{this.state.campaign.address}</Table.Cell>
                    </Table.Row>
                    ...
                    <Table.Row>
                        ...
                        <Table.Cell singleLine>{this.state.campaign.deadline.toString()}</Table.Cell>
                    </Table.Row>

                    <Table.Row>
                        <Table.Cell singleLine>I am beneficiary</Table.Cell>
                        <Table.Cell singleLine>{this.state.campaign.isBeneficiary.toString()}</Table.Cell>
                    </Table.Row>
                ...
                <Table.Footer fullWidth>
                    <Table.Row>
                        <Table.HeaderCell colSpan="2">{this.campaignInteractionSection()}</Table.HeaderCell>
                    </Table.Row>
                </Table.Footer>
            </Table>
        </div> 
    ...
    contributeInterface() {
        return <div>
            <Input action = {{ color: 'teal', content: 'Contribute', onClick:this.onContribute }}
            actionPosition='left' label='ETH' labelPosition='right' placeholder='1' 
    ...
    async onContribute(event) {
        ...
        if(accounts[0] == null || amount == null) {
            alert(`Contributing ${amount} to a account ${accounts[0]} failed`)
            return;
        }
        ...
        campaign.totalCollected += Number.parseInt(amount);
